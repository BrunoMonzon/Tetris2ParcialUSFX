...
ABoard::ABoard()
{
    ...
    //SINGLETON
    TArray<AActor*> Instancias; //Array de actores Board  
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), ABoard::StaticClass(), Instancias); //Obtener todos los actores de la clase Board

    if (Instancias.Num() > 1)
    {
        //If exist at least one of them, set the instance with the first found one
        // Si existe al menos una instancia, establezca la instancia con el primero encontrado
