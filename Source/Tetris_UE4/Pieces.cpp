...
void APieces::SpawnBlocks()
{
    ...
    for(auto&& YZ: YZs)
    {
       ...
       /* ABlock* B = GetWorld()->SpawnActor<ABlock>(this->GetActorLocation(), Rotation);
        ...
        int32 BlockType = FMath::RandRange(0, 4);
        ABlock* B;
        switch (BlockType)
        {
			case 0:
				B = GetWorld()->SpawnActor<ABlockAgua>(this->GetActorLocation(), Rotation);
                B-> Mesh->SetMaterial(1, Colors[0]);
				break;
			case 1:
				B = GetWorld()->SpawnActor<ABlockDulce>(this->GetActorLocation(), Rotation);
                B-> Mesh->SetMaterial(1, Colors[1]);
				break;
            case 2:
                B = GetWorld()->SpawnActor<ABlockFuego>(this->GetActorLocation(), Rotation);
				B-> Mesh->SetMaterial(1, Colors[2]);
                break;
            case 3:
                B = GetWorld()->SpawnActor<ABlockGoma>(this->GetActorLocation(), Rotation);
                B-> Mesh->SetMaterial(1, Colors[3]);
                break;
            case 4:
				B = GetWorld()->SpawnActor<ABlockNatural>(this->GetActorLocation(), Rotation);
...
void APieces::setBlockAgua()
{
    GetWorld()->SpawnActor<ABlockAgua>(this->GetActorLocation(), FRotator(0.0, 0.0, 0.0));
}

void APieces::setBlockDulce()
{
	GetWorld()->SpawnActor<ABlockDulce>(this->GetActorLocation(), FRotator(0.0, 0.0, 0.0));
}

void APieces::setBlockFuego()
{
	GetWorld()->SpawnActor<ABlockFuego>(this->GetActorLocation(), FRotator(0.0, 0.0, 0.0));
}

void APieces::setBlockGoma()
{
	GetWorld()->SpawnActor<ABlockGoma>(this->GetActorLocation(), FRotator(0.0, 0.0, 0.0));
}

void APieces::setBlockNatural()
{
	GetWorld()->SpawnActor<ABlockNatural>(this->GetActorLocation(), FRotator(0.0, 0.0, 0.0));
}
